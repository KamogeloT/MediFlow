...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'queue' AND column_name = 'is_appointment_based') THEN
    ...
    RAISE NOTICE 'Added is_appointment_based column to queue table';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'queue' AND column_name = 'appointment_id') THEN
    ALTER TABLE public.queue ADD COLUMN appointment_id uuid;
    RAISE NOTICE 'Added appointment_id column to queue table';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'queue' AND column_name = 'appointment_time') THEN
    ...
    RAISE NOTICE 'Added appointment_time column to queue table';
  END IF;
  
  -- Add estimated_wait_time column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'queue' AND column_name = 'estimated_wait_time') THEN
    ALTER TABLE public.queue ADD COLUMN estimated_wait_time integer DEFAULT 0;
    RAISE NOTICE 'Added estimated_wait_time column to queue table';
  END IF;
  
  -- Add is_walk_in column if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'queue' AND column_name = 'is_walk_in') THEN
    ALTER TABLE public.queue ADD COLUMN is_walk_in boolean DEFAULT false;
    RAISE NOTICE 'Added is_walk_in column to queue table';
  END IF;
END $$;
...
-- Create queue_archive table if it doesn't exist
...
-- =====================================================
-- STEP 3: CREATE FUNCTIONS (AFTER COLUMNS EXIST)
-- =====================================================

-- Create a function to convert appointments to queue items for current date
...
-- Create a function to archive expired queue items (older than 24 hours)
...
-- Create a function to get current queue items (filtered by doctor's departments)
...
-- Create a function to get upcoming appointments (not yet in queue)
...
-- Create a function to get past appointments (for archiving)
...
-- Create a function to manually add walk-in patients to queue
...
-- =====================================================
-- STEP 4: CREATE INDEXES AND UPDATE DATA
-- =====================================================
...
-- Update existing queue items to mark them as non-appointment based
UPDATE public.queue 
...
-- =====================================================
-- STEP 6: RUN INITIAL MAINTENANCE
-- =====================================================
